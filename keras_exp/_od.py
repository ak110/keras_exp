...
class ObjectDetector(object):
    ...
    def create_model(self):
        ...
        # ベースネットワーク
        x = inputs = keras.layers.Input(self.input_size + (3,))
        ...
        def _conf(x, fm_count, size_ix, ar_ix):
            ...
            x = _conv_bn_act(x, filters * 2, (1, 1), padding='same', activation='elu',
                             name='pm{}-{}-{}-conf_sq'.format(fm_count, size_ix, ar_ix))
            for depth in range(size_ix):
                x = _conv_bn_act(x, filters * 2, (3, 3), padding='same', activation='elu',
                                 name='pm{}-{}-{}-conf_conv{}'.format(fm_count, size_ix, ar_ix, depth))
            ...
            x = keras.layers.Reshape((-1, self.nb_classes), name='pm{}-{}-{}-conf_reshape'.format(fm_count, size_ix, ar_ix))(x)
        ...
        def _loc(x, fm_count, size_ix, ar_ix):
            filters = 4
            x = _conv_bn_act(x, filters * 2, (1, 1), padding='same', activation='elu',
                             name='pm{}-{}-{}-loc_sq'.format(fm_count, size_ix, ar_ix))
            for depth in range(size_ix):
                x = _conv_bn_act(x, filters * 2, (3, 3), padding='same', activation='elu',
                                 name='pm{}-{}-{}-loc_conv{}'.format(fm_count, size_ix, ar_ix, depth))
            x = keras.layers.Conv2D(filters, (3, 3), padding='same',
                                    name='pm{}-{}-{}-loc'.format(fm_count, size_ix, ar_ix))(x)
            x = keras.layers.Reshape((-1, 4), name='pm{}-{}-{}-loc_reshape'.format(fm_count, size_ix, ar_ix))(x)
            return x

        confs, locs = [], []
        for fm_count in ObjectDetector.FM_COUNTS:
            for size_ix in range(len(self.pb_size_ratios)):
                for ar_ix in range(len(self.aspect_ratios)):
        ...
        # model.compile('nadam', self.loss, [self.loss_loc, self.acc_bg, self.acc_obj])
        model.compile(keras.optimizers.SGD(momentum=0.9, nesterov=True), self.loss, [self.loss_loc, self.acc_bg, self.acc_obj])
        return model
